...
eseguite: **O9, O10, O11** (O1 al giro 79, O2 al 80, O3 al 81, O4 al 82, O5 al 83, O6 al 84, O7 al 85, O8 al 86; O9 nata al 84, O10 al 85, O11 al 86).
...
| **O8 volatilità dell'alfa** | 86 | **FALSIFICATA sul ramo 1: la volatilità dell'alfa non spiega niente.** Griglia a rotazione 3,5× e alfa geometrico 6, con volatilità 0/2/5/10/15/20 e 200 estrazioni per cella: mediane **+1,55 / +1,56 / +1,56 / +1,49 / +1,22 / +1,37** — **non monotona** e con una caduta massima di **0,33**, che a vol 20 scende a **0,18**, sotto la soglia di 0,2 che falsificava. La non-monotonia è **rumore Monte Carlo**: con 200 estrazioni l'errore standard della mediana è ±0,16 a vol 15 e ±0,23 a vol 20, quindi l'effetto è dello stesso ordine del proprio rumore. Alla volatilità **vera** del candidato — **12,64** punti, misurata e non assunta, contro i «circa 14» della voce — la perdita interpolata è **0,21**. **E il giro trova un errore mio nel giro 83**: gli 0,8 punti da spiegare **non esistevano**. Avevo confrontato l'alfa del candidato misurato come **differenza di CAGR** (17,28 − 10,78 = **6,50**, aritmetico) col parametro `a` del sintetico, che è **geometrico** (1,1728/1,1078 − 1 = **5,87**). Il sintetico all'alfa giusto dà **+1,45** contro il **+1,18** reale: **scarto 0,27, non 0,8** — e la volatilità ne spiega 0,21, lasciandone **sei centesimi**. **Correzione a un numero registrato**: il tasso di cambio vero a 3,5× è **≈5,5**, non «6,0-6,5» come avevo scritto al giro 83. La curva era quasi giusta e l'avevo screditata su un confronto mal fatto. Resta vero, ma un quarto della taglia dichiarata, che il sintetico è un limite inferiore. Osservazione collaterale: a vol 15 la mediana è +1,22 ma il **p5 è −1,75** e il p95 **+4,25** — **la mediana non è l'esperienza** |
| **O7 il fattore N_eff/N** | 85 | **confermata sul ramo che falsifica, due clausole descrittive su tre sbagliate.** I quattro fattori: **0,300** (skip×taglia, N=20, corr 0,761), **0,500** (dodici candidati, N=12, corr 0,412), **0,400** (posizioni×frequenza, N=15, corr 0,654), **0,340** (calendari×taglia, N=47, corr 0,466). Non è una costante — ampiezza 0,200 — ma il rapporto max/min è **1,67×**, non «sopra 3», e i **calendari fanno 0,340 e non «sotto 0,15»**. L'errore è di **classificazione, non di taglia**: avevo descritto la famiglia 4 come «varia solo il mese di ribilanciamento», ma è **quattro strategie × dodici calendari** e la sua correlazione media (0,466) è la **seconda più bassa** delle quattro — avevo chiamato la più omogenea quella che è la seconda più eterogenea. **La struttura non prevista**: `N_eff` vale **6,00 / 6,00 / 6,00 / 16,00** — tre famiglie di 12, 15 e 20 celle con correlazioni da 0,41 a 0,76 danno **lo stesso numero di test indipendenti** (Nyholt conferma: 9,04 / 8,65 / 8,79, entro il 4%). Il meccanismo che avevo dato — «il fattore scala con l'eterogeneità» — è **quasi assente**: il rapporto varia perché varia il **denominatore**. **Allargare una griglia da 12 a 20 celle non aggiunge test indipendenti.** **Controllo incrociato riuscito**: la famiglia 1 dà 0,300 nel motore mensile e 0,300 in quello giornaliero (giro 82), scarto **zero**. Bug trovato eseguendo: nella famiglia 4 la cella «EW gennaio» **è** il benchmark, extra identicamente nullo — scartata, e **una delle 48 configurazioni del giro 68 era il benchmark**. Per il cancello: la soglia sulla famiglia del candidato va da **+1,50** a **+1,9** su tutta la banda dei fattori osservati, e il **+1,18 è respinto ovunque** |
| **O6 statistiche di allarme** | 84 | **confermata sul ramo che falsifica (1 statistica su 5), e la predizione principale è sbagliata in pieno.** **La pendenza NON segnala il candidato**: è **+0,40**, cioè il profilo decennale *sale*, e il candidato è **settimo su tredici** — a metà classifica. Sul t-stat dell'ultimo terzo è **dodicesimo su tredici**, quasi il migliore del gruppo. L'unica che alza la mano è la **(2) differenza fra i terzi** (−4,10, rango 3/13), che però ne segnala **altri sei su dodici**: non distingue, segnala metà del campo. **L'errore che il giro trova è mio**: al giro 77 avevo messo a verbale, prima dell'holdout, che «le uniche due finestre negative sono le ultime due, il degrado punta verso l'holdout» — trasformata in statistica **non discrimina**, perché il profilo è una **gobba** (+3,98 → +6,53 nel 1993 → −3,39) e una retta ci passa con pendenza positiva. Avevo letto un trend nella coda di una serie rumorosa: **lo stesso errore documentato quattro volte, in immagine speculare** — il *minimo* di N celle preso per un presagio invece del massimo preso per un risultato. Che l'holdout mi abbia dato ragione non rende l'osservazione corretta: era giusta per caso. **Chiusura dell'autopsia**: non si può giudicare un candidato guardando il candidato, solo la **griglia da cui viene**. Il cancello sul margine del giro 79 non è una difesa fra tante, è **l'unica**. La batteria funziona però sui low-vol (H4 fa 5 allarmi su 5), che erano davvero deteriorati |
| **O5 tasso di cambio** | 83 | **confermata sul ramo che falsifica.** La curva `alfa lordo richiesto per +1,00 netto` in funzione della rotazione: **1,57** a 0,2×, **2,04** a 0,5×, **2,38** a 0,9×, poi **4,39** a 1,1×, 4,85 a 2×, **5,27** a 3,5×, 5,78 a 6×. **Il salto attraverso la soglia del 100% vale +2,01 punti di alfa richiesto** — più che quadruplicare la rotazione da 0,2× a 0,9×, che ne costa 0,81. Rapporto 3,5×/0,5× = **2,58×**, sopra il 2 che falsificava. Clausola descrittiva **sbagliata**: sotto la soglia servono **2,04 e 2,38** punti a 0,5× e 0,9×, non «meno di 2» — cioè **il conto è già di due a uno anche al 33%**, e la soglia lo rende soltanto proibitivo. Colonna a=0: ruotare **senza alfa** costa da −0,51 (0,2×) a −3,40 (6×), che è il prezzo del movimento in sé. **Cautela dal controllo col giro 80**: il candidato reale aveva 6,50 di alfa a 3,28× e ha reso +1,18, mentre il sintetico a 3,5× con 6,50 darebbe ~+1,9 — **il sintetico è ottimista di ~0,8 punti** perché impone alfa costante mentre quello vero è volatile (21,07% contro 16,07% di volatilità). La curva è quindi un **limite inferiore** → voce **O8**. **[CORRETTO AL GIRO 86: lo scarto era 0,27 e non 0,8 — avevo confrontato un alfa aritmetico (6,50) col parametro geometrico del sintetico (5,87). Il tasso vero a 3,5× è ≈ 5,5, non 6,0-6,5.]** Riassunto del progetto in un numero: **per battere il PAC di un punto netto ruotando come un momentum concentrato serve un alfa lordo di sei punti l'anno**, e il miglior candidato mai prodotto ne aveva 6,50 in campione e **0,31** fuori. Errore aritmetico nella voce dichiarato prima: «quattordici combinazioni × sei alfa = 42» è incoerente (7×6=42), ho usato la griglia che il totale implica |
| **O4 N effettiva** | 82 | **confermata sul ramo che falsifica.** Correlando le serie di **extra** (scelta dichiarata prima: i rendimenti grezzi misurerebbero solo che sono tutti azionari) la correlazione media è **0,750** e il primo autovalore spiega il **76%**. **`N_eff` = 6,00 esatto** con Li & Ji — e l'intero non è coincidenza: su una matrice di correlazione la traccia vale *M*, quindi la somma delle parti frazionarie è `M − Σ⌊λ⌋`. Soglie: nominale **+2,19**, con `N_eff` **+1,495**, Nyholt +1,76 — **il candidato (+1,18) resta respinto sotto tutte**. **I tre stimatori non concordano**: il rapporto di partecipazione dà 1,68 e lo **farebbe passare**; non lo prendo perché misura il numero effettivo di *fattori*, non di *test*, ma la scelta dello stimatore è **il quarto grado di libertà** che il progetto incontra. Griglia implicita col fattore di riduzione 0,300: **+2,78** come scritta nella voce e **+2,14** con la griglia difendibile — entrambe sopra +2,0. **Sovraconteggio nella voce dichiarato prima**: il «×12 calendari» non si applica a strategie mensili, che non hanno quel grado di libertà. Risultato collaterale: **i due errori del giro 79 si elidono a cinque centesimi** (+2,19 nominale contro +2,14 corretto per entrambi) — il numero pubblicato allora era accidentalmente quasi giusto. Clausola descrittiva sbagliata: la soglia corretta è **+1,495**, cioè fuori dalla banda [+1,5; +1,9] per **cinque millesimi**, e la registro come sbagliata invece di arrotondarla |
| **O3 finestre disgiunte** | 81 | **FALSIFICATA su entrambe le clausole, e il rimedio che proponevo era peggiore del difetto.** Con finestre decennali **disgiunte** (1970-79 +4,81, 1980-89 +0,22, 1990-99 +4,42, 2000-09 −3,39) il candidato fa **75,0%** — sopra i 2/3, quindi **G3 passa anche su prove indipendenti** e la condizione di falsificazione scatta. Seconda clausola sbagliata pure: il bootstrap a blocchi dà un intervallo largo **29,0%** contro il **20,1%** di Wilson su n=29, cioè **1,45×** e non «almeno 3» — la sovrapposizione vale **n_eff ≈ 14** su 29, uno sconto reale ma metà di quello che avevo scritto, e la frase «lo stesso pezzo di storia contato ventisette volte» era falsa. **Il rimedio è dannoso**: con 4 finestre e soglia 2/3 servono 3/4, e sotto H0 il caso passa il **31,25%** delle volte contro il **3,07%** delle mobili — **dieci volte peggio**. Si vede anche empiricamente: sui dodici candidati del giro 72 passano **2/12 con le mobili e 4/12 con le disgiunte**, con **4 verdetti su 12 che si ribaltano**. Il limite vero è **di campione, non di schema**: quarant'anni contengono quattro decenni indipendenti, e nessuna scelta di finestre ne produce di più. **Il colpevole era già stato trovato al giro 79**: il candidato andava respinto dalla **selezione** (soglia sul margine +2,19 contro +1,18), non dalla stabilità — O3 assolve il secondo sospettato. Resta però un grado di libertà mai dichiarato: **lo schema delle finestre cambia il verdetto di un cancello in 4 casi su 12** |
| **O2 scomposizione train-holdout** | 80 | **confermata, entrambe le clausole.** Scomposizione additiva del divario −2,12 in `alfa lordo − zavorra_cand + zavorra_bench`: **alfa lordo −6,19** (da +6,50 a +0,31, cioè il **291%** del divario, stessa forma del 398% del giro 60), zavorra del candidato +2,25, zavorra del benchmark +1,82. **Rotazione aggiuntiva: −0,11 punti** contro il «sotto 0,30» previsto e lo 0,50 che falsificava — mezza rotazione in più su una base già al 52% costa quasi niente, che è il giro 66 visto dal lato opposto. **Il candidato ha PERSO, non è stato superato**, ma per 59% contro 41%: cade di −3,63 punti di CAGR lordo mentre il benchmark sale di +2,56. Doppio conteggio nella voce dichiarato **prima** di eseguire: (a) alfa lordo e (c) livello del benchmark sono la stessa differenza guardata due volte, e le ho sciolte nelle **due metà di Δ(alfa)**, che è ciò che la clausola di falsificazione chiedeva. Risultato collaterale: **nel train il candidato aveva 6,50 punti di alfa lordo e ne consegnava 1,18** — zavorra 6,01 contro 0,69 del benchmark, differenziale **5,32 punti**. E **la macchina fiscale è un ammortizzatore simmetrico**: l'alfa crolla di 6,19 e il netto solo di 2,12, perché la zavorra si riduce insieme all'alfa. Chi guarda solo l'IRR vede una strategia indebolita; chi guarda il lordo vede un segnale morto. Cautela dichiarata: la zavorra **non è un'aliquota**, scala col livello del lordo → voce **O5** |
| **O1 cancello sul margine** | 79 | **confermata, tutte e tre le clausole.** La versione **margine** della statistica di Bailey-Lopez de Prado — stessa formula di G2, applicata alla dispersione dei **margini di IRR netta** invece che degli Sharpe — dà soglia **+2,19** sulla famiglia di 20 celle (σ 1,15, ampiezza 4,27) e **respinge il candidato**, che aveva +1,18. G2 come scritto lo faceva passare (SR 0,1755 contro soglia 0,0608). Il verdetto **non dipende dalla centratura**: anche la soglia centrata sulla media della famiglia (+1,33) lo respinge. **Rapporto fra le due esigenze 5,36×** contro il «sopra 3» previsto. **Zero passanti su 32** fra le due famiglie, e il massimo vero della famiglia B non è il candidato ma il **12-1 top-3 a +1,51**, respinto anche lui: il cancello non è tarato sul caso che ha fallito. **Controllo superato**: l'equal-weight contro il cap-weight (+1,50, non selezionato, N=1) passa, quindi il cancello discrimina ancora — sotto la lettura di stress (N=20) verrebbe respinto anche lui, e lo riporto. Risultato non cercato: **G1 era tarato sulla cosa sbagliata** — la soglia di +1,00 fu fissata sul rumore di misura, mentre il riferimento giusto è la dispersione che la ricerca genera, che cresce con N: **+1,91** a 12 celle, **+2,19** a 20, **+3,25** a 240 (la griglia che il progetto ha davvero esplorato). **G1 e G2 provavano a fare la stessa cosa e nessuno dei due la faceva**; il cancello sul margine li unifica. Limite dichiarato: la formula assume estrazioni indipendenti e le celle non lo sono → voce **O4** |
...
## M. Il primo candidato che arriva ai cancelli

**M1 — Il 12-3 top-5 mensile contro i quattro cancelli, misurati tutti**
Nata al giro 76. È la prima cella in settantasei giri che passa **tre cancelli su
quattro** della regola del giro 72: G1 con **+1,18** di margine, G2 con **DSR
0,9870** (`var_sr` sulla famiglia del giro, come la regola prescrive) e G4 per
costruzione, essendo mensile. **G3 non è mai stato misurato**, e G1 è stato
misurato contro un benchmark su **un solo calendario** — gennaio.
Quest'ultimo è un buco della regola, trovato applicandola: una strategia mensile
non ha il grado di libertà del calendario, ma il suo **benchmark annuale sì**, e
sia il giro 72 sia il giro 76 hanno usato il solo gennaio. La mediana di G1 va
presa sui dodici calendari **del benchmark**.
Da misurare, sul motore giornaliero del giro 73, finestra 1969-2009:
**(a)** il margine del 12-3 top-5 mensile contro l'equal-weight annuale su **tutti
e dodici** i calendari del benchmark, prendendone la **mediana** — è quella che
deve superare +1,00;
**(b)** **G3**: extra positivo in ≥ 2/3 delle finestre decennali mobili **e**
mediana degli extra > 0;
**(c)** il **DSR** con `var_sr` stimata su una famiglia che contenga **anche lo
skip** (5 skip × 4 taglie = 20 celle, non 10), perché il giro 76 ha mostrato che
lo skip è un parametro libero e una famiglia che lo ignora sottostima la
dispersione che la ricerca genera.
*Predizione*: **(a) tiene** — la mediana sui dodici calendari resta **sopra
+1,00** e si sposta di **meno di 0,3 punti** da +1,18, perché il giro 68 ha
misurato che l'ampiezza del calendario per un **equal-weight** è appena **0,14%**:
il benchmark non ha il grado di libertà che ha un portafoglio concentrato.
**(b) fallisce**: la quota di finestre decennali positive resta **sotto i 2/3**,
perché il candidato più vicino mai registrato (12-2 top-5 mensile, giro 72) si
fermò a **66,0% contro 66,7%** e perché il giro 59 ha mostrato che allungando
l'orizzonte tutti i candidati peggiorano. **(c) scende ma regge**: il DSR resta
**sopra 0,95**, perché allargare la famiglia da 10 a 20 celle alza `var_sr` meno
di quanto servirebbe a ribaltarlo.
Esito atteso: **tre cancelli su quattro, l'holdout resta sigillato**, e il
progetto chiude avendo mostrato che il suo miglior candidato fallisce sulla
**stabilità nel tempo**, non sul margine.
*Falsificata se*: il candidato **passa tutti e quattro** i cancelli — nel qual
caso l'holdout va aperto **su quello solo, una volta sola**, il risultato si
registra qualunque sia, e il progetto finisce lì. **Oppure** se (a) cade, cioè la
...
Nata al giro 79. Il cancello sul margine usa la formula del massimo atteso di *N*
estrazioni **indipendenti**, e le celle di una famiglia non lo sono: condividono
universo, finestra e struttura del segnale. I due errori vanno in direzioni
opposte e nessuno dei due è quantificato. **Verso il basso**: venti celle
correlate hanno un massimo atteso più piccolo di venti indipendenti, quindi
+2,19 è conservativo. **Verso l'alto**: la *N* vera della ricerca non è venti ma
l'intera griglia implicita — 5 skip × 12 calendari × 4 taglie × 3 frequenze — e
passare da 20 a 240 celle sposta la soglia da +2,19 a +3,25.
Da misurare, sul solo 1969-2009: **(a)** la matrice di correlazione dei rendimenti
mensili netti delle venti celle del giro 77, e da lì il **numero effettivo di
estrazioni indipendenti** `N_eff` (via autovalori della matrice, come si fa per i
test multipli correlati); **(b)** la soglia ricalcolata con `N_eff` al posto di
*N*; **(c)** la soglia con la griglia implicita completa, contando ogni grado di
libertà che il progetto ha scoperto **dopo** averlo già usato (skip al giro 76,
calendario al giro 68).
*Predizione*: `N_eff` è **fra 3 e 8** su 20 celle nominali — le celle sono molto
correlate perché quattro taglie dello stesso segnale sono quasi lo stesso
portafoglio — e la soglia corretta scende a **+1,5/+1,9**, quindi **il candidato
resta respinto** (+1,18), ma con meno margine di quanto il giro 79 suggerisse. Con
la griglia implicita completa e la stessa correzione, la soglia **risale sopra
+2,0**: i due errori non si compensano, quello della *N* è più grande.
*Falsificata se*: la soglia corretta scende **sotto +1,18**, cioè il candidato
sarebbe passato anche col cancello giusto — nel qual caso O1 aveva ragione per il
motivo sbagliato e il fallimento dell'holdout torna inspiegato — **oppure** se
`N_eff` supera 15, cioè le celle sono quasi indipendenti e l'obiezione della
correlazione non esisteva.

**O5 — Il tasso di cambio fra alfa lordo e margine netto**
Nata al giro 80. La scomposizione ha mostrato che nel train il candidato aveva
**6,50 punti di alfa lordo** e ne consegnava **1,18**, con una zavorra di 6,01
contro 0,69 del benchmark. Ma quel differenziale di 5,32 punti **non è
un'aliquota**: mescola due cose diverse — la tassa sulla **rotazione** e la tassa
sull'**aver guadagnato di più**, perché una strategia che rende il 17,28% lordo ha
in valore assoluto più plusvalenze da realizzare di una che rende il 10,78%.
Separarle produce il numero più operativo che il progetto possa dare: **quanti
punti di alfa lordo servono per battere il benchmark di un punto netto, in
funzione della rotazione**.
Da misurare, sul solo 1969-2009: una griglia di flussi **sintetici** costruiti
sopra il benchmark equal-weight annuale, con alfa lordo costante imposto a
0/1/2/4/6/8 punti l'anno e rotazione imposta a 0,2×/0,5×/0,9×/1,1×/2×/3,5×/6×
(quattordici combinazioni × sei livelli di alfa = 42 celle, tutte pre-dichiarate,
nessuna selezione). Per ogni cella, il margine netto di IRR. Da lì la **curva del
tasso di cambio** `alfa lordo richiesto per +1,00 netto` in funzione della
rotazione, e il salto attraverso la soglia del 100%.
*Predizione*: il tasso di cambio è **fortemente non lineare nella rotazione e
discontinuo alla soglia**. Sotto 1,0× serve **meno di 2 punti** di alfa lordo per
farne uno netto; sopra 1,0× ne servono **più di 3**; e a 3,5× — la rotazione del
candidato — ne servono **fra 4 e 6**, coerente con i 6,50 → 1,18 misurati al giro
80. Il salto attraverso la soglia vale **almeno 1,0 punto** di alfa richiesto, cioè
è più grande di qualunque effetto di secondo ordine misurato nel progetto.
*Falsificata se*: il tasso di cambio è **quasi lineare** nella rotazione, cioè il
rapporto fra la richiesta a 3,5× e quella a 0,5× sta **sotto 2** — nel qual caso
la rotazione è un costo proporzionale e non il vincolo dominante che ottanta giri
hanno descritto — **oppure** se il salto alla soglia vale **meno di 0,5 punti**,
cioè il gradino fiscale conta meno del rumore di misura.

**O6 — Esiste una statistica in campione che avrebbe segnalato il candidato?**
Nata al giro 81. Il quadro dopo tre giri di autopsia: **G1 e G2 lo avrebbero
fermato** con la soglia giusta (giro 79), **G3 no in nessuna versione** (giro 81),
e il crollo del lordo da +6,50 a +0,31 (giro 80) non era visibile in campione per
definizione. Resta una domanda sola: fra le statistiche calcolabili **sul solo
train**, ce n'è una che avrebbe alzato la mano — al di là del cancello sulla
selezione, che non guarda il candidato ma la griglia?
Il sospetto è il **profilo temporale**: al giro 77 avevo messo a verbale che le
uniche due finestre decennali negative erano **le ultime due** (1999-2008 a −1,23,
2000-2009 a −3,39), dopo cinque consecutive sopra +3. Quello è un **trend**, e
nessun cancello lo guardava.
Da misurare, sul solo 1969-2009, applicando a **tutti e tredici** i candidati (i
dodici del giro 72 più il 12-3 top-5) una batteria di cinque statistiche
...
**(3)** quota di finestre positive calcolata **solo sull'ultimo decennio** di
finestre; **(4)** t-stat dell'extra mensile sull'ultimo terzo; **(5)** rapporto
fra il numero di mesi positivi negli ultimi cinque anni e nei primi cinque.
*Predizione*: **la pendenza (1) segnala il candidato**, ed è **l'unica** delle
cinque a farlo con specificità — cioè segnala il 12-3 top-5 e **non più di tre**
degli altri dodici. Le statistiche (3) e (5) sono troppo rumorose per discriminare
(segnaleranno più di metà dei candidati o nessuno), e la (4) non segnala perché il
t-stat mensile è dominato dalla volatilità e non dal trend.
*Falsificata se*: **nessuna** delle cinque segnala il candidato — nel qual caso
il fallimento dell'holdout non era visibile in campione con nessuno strumento e la
selezione (giro 79) resta l'unica difesa possibile — **oppure** se **più di tre**
delle cinque lo segnalano, cioè segnalare era facile e il progetto ha semplicemente
guardato dalla parte sbagliata per settantasette giri.

**O7 — Il fattore 0,300 è una proprietà di quella famiglia o una costante?**
Nata al giro 82. Il cancello sul margine, corretto per la correlazione, dipende da
un solo numero: `N_eff/N`, misurato **0,300** su **una sola** famiglia — le venti
celle skip × taglia del giro 77. Un cancello che poggia su una quantità misurata
una volta sola non è un cancello: se il fattore varia molto da famiglia a
famiglia, la soglia va ristimata ogni volta e il cancello non è applicabile a
priori; se è stabile, diventa una costante di progetto.
Da misurare, sul solo 1969-2009, il rapporto `N_eff/N` (Li & Ji sulle serie di
extra, come al giro 82) su **quattro** famiglie già esistenti, di eterogeneità
crescente: **(1)** le 20 celle skip × taglia del giro 77 (già fatto, 0,300);
**(2)** i **12 candidati** del giro 72, che mescolano momentum, low-vol e
punteggio misto su due frequenze; **(3)** la griglia **F2** posizioni × frequenza
del giro 50, 15 celle dello stesso segnale; **(4)** i **48 calendari × taglia** del
giro 68, che variano solo il mese di ribilanciamento.
*Predizione*: il fattore **non è costante e scala con l'eterogeneità della
famiglia**. Il valore più basso è quello dei **calendari** (famiglia 4), sotto
**0,15**, perché dodici calendari dello stesso portafoglio sono quasi lo stesso
test; il più alto è quello dei **dodici candidati** (famiglia 2), sopra **0,45**,
perché momentum e low-vol sono segnali diversi. L'intervallo fra il minimo e il
massimo è **superiore a un fattore 3**, quindi `N_eff/N` **non è utilizzabile come
costante** e va ristimato per famiglia.
*Falsificata se*: tutti e quattro i fattori stanno **entro ±0,05** l'uno
dall'altro — nel qual caso 0,300 è una costante di progetto e il cancello sul
margine diventa applicabile senza rimisurare — **oppure** se l'ordinamento è
invertito, cioè la famiglia dei calendari risulta **più** indipendente di quella
dei dodici candidati, il che vorrebbe dire che `N_eff` non misura l'eterogeneità
che credo misuri.

**O8 — Quanto costa che l'alfa sia volatile invece che costante**
Nata al giro 83. La curva del tasso di cambio è costruita su un alfa **costante
ogni mese**, e il controllo col giro 80 mostra che è **ottimista di circa 0,8
punti** di margine netto: il candidato reale, con 6,50 di alfa lordo a 3,28× di
rotazione, ha consegnato **+1,18**, mentre il sintetico a 3,5× con lo stesso alfa
ne darebbe circa **+1,9**. Il sospetto è la **volatilità dell'alfa**: il candidato
ha il 21,07% di volatilità contro il 16,07% del benchmark, e un vantaggio grumoso
realizzato con rotazione alta paga imposte nei momenti sbagliati. Finché non è
misurato, tutta la curva del giro 83 è un limite inferiore di taglia ignota.
Da misurare, sul solo 1969-2009: rifare la griglia del giro 83 alla sola rotazione
**3,5×** e al solo alfa lordo **6,0**, ma con l'alfa mensile **rumoroso** invece
che costante — media imposta a 6 punti l'anno e volatilità dell'alfa imposta a
**0 / 2 / 5 / 10 / 15 / 20** punti annualizzati (sei celle, tutte pre-dichiarate),
con **200 estrazioni** per cella e seme fissato. Riportare la mediana e i
percentili 5-95 del margine netto, e da lì **di quanto sale l'alfa richiesto per
+1,00** a ciascun livello di volatilità.
*Predizione*: il margine netto **scende in modo monotono** al crescere della
volatilità dell'alfa, e alla volatilità che il candidato aveva davvero — circa
**14 punti** di tracking error annualizzato, cioè fra la cella 10 e la cella 15 —
la perdita è **fra 0,5 e 1,2 punti**, coerente con lo 0,8 misurato indirettamente
al giro 83. Il meccanismo è fiscale e non statistico: la dispersione **non**
dovrebbe far scendere la mediana se il motore fosse lineare, quindi la caduta
misura la **convessità del prelievo**.
*Falsificata se*: la mediana del margine **non scende** al crescere della
volatilità dell'alfa, o scende di **meno di 0,2 punti** anche a 20 di volatilità —
nel qual caso lo scarto di 0,8 del giro 83 ha un'altra causa e la curva del tasso
di cambio è più affidabile di quanto ho scritto — **oppure** se la caduta supera
**2,5 punti**, cioè la volatilità dell'alfa conta più della rotazione e il
progetto ha passato ottantatré giri a guardare la variabile sbagliata.

**O9 — La batteria di allarme misura il deterioramento o solo il margine?**
Nata al giro 84. Guardando la tabella degli allarmi viene un sospetto: i candidati
con molti allarmi sono quelli che **perdevano già in campione**. H4 low-vol fa
cinque allarmi su cinque e ha un margine di **−1,56**; i due low-vol annuali ne
fanno quattro con margini −1,12 e −0,61; il candidato e il 12-2 top-5 mensile, i
due margini più alti (**+1,18** e **+0,56**), ne fanno **uno** ciascuno. Se è così,
la batteria non è un sistema di allarme precoce: è **una misura rumorosa del
margine**, e non aggiunge niente a G1.
Da misurare, sul solo 1969-2009, sui tredici candidati già calcolati al giro 84:
**(a)** la correlazione di Spearman fra il **numero di allarmi** e il **margine in
campione**; **(b)** la stessa correlazione per **ciascuna** delle cinque
statistiche presa singolarmente, per vedere se qualcuna è ortogonale al margine;
**(c)** la **correlazione parziale** fra ciascuna statistica e il margine
controllando per la rotazione, perché low-vol e momentum hanno rotazioni molto
diverse e la rotazione muove il margine per conto suo (giro 83: da 1,57 a 5,78
punti di alfa richiesto).
*Predizione*: la correlazione fra numero di allarmi e margine è **negativa e
forte, sotto −0,70**, cioè la batteria **replica G1** invece di aggiungerci
qualcosa. Almeno **una** delle cinque — la mia scommessa è la **(1) pendenza**, che
è l'unica costruita su una *derivata* e non su un livello — resta **sotto 0,40 in
valore assoluto**, cioè porta informazione propria pur non avendo segnalato il
candidato. Controllando per la rotazione le correlazioni **si attenuano di almeno
0,15**, perché parte del legame passa dal canale fiscale e non dal segnale.
*Falsificata se*: la correlazione complessiva sta **sopra −0,40** in valore
assoluto, cioè la batteria è davvero indipendente dal margine e l'idea di un
allarme precoce non va chiusa — **oppure** se **nessuna** delle cinque statistiche
è ortogonale al margine (tutte sopra 0,40 in valore assoluto), nel qual caso non
c'è niente da salvare e la famiglia degli indicatori di deterioramento va chiusa
in blocco.

**O10 — C'è un tetto ai test indipendenti estraibili da 49 settori?**
Nata al giro 85. Il risultato non richiesto di quel giro: `N_eff` vale **6,00** su
tre famiglie di **12, 15 e 20** celle con correlazioni medie da **0,41 a 0,76**, e
sale solo a **16,00** su una da 47. Nyholt conferma (9,04 / 8,65 / 8,79, entro il
4%). Sembra esserci un **tetto** al numero di scommesse davvero diverse che si
costruiscono su questo universo, e le griglie del progetto ci sbattevano contro
senza saperlo. Se il tetto è reale, ha una conseguenza pratica grossa: **allargare
una ricerca non la rende più esplorativa, la rende solo più costosa in soglia**.
Da misurare, sul solo 1969-2009, `N_eff` (Li & Ji e Nyholt sulle serie di extra
contro l'equal-weight annuale di gennaio, come al giro 85) su famiglie costruite
apposta a **diversità crescente di segnale**, tutte a 12 celle per tenere N fisso:
**(a)** 12 varianti di **momentum** (skip × taglia); **(b)** 6 momentum + 6
**low-vol**; **(c)** 4 momentum + 4 low-vol + 4 **reversal a 1 mese**; **(d)** 3
momentum + 3 low-vol + 3 reversal + 3 **equal-weight a frequenze diverse**. Poi la
curva di `N_eff` in funzione del **numero di segnali distinti** (1, 2, 3, 4).
*Predizione*: `N_eff` **cresce ma satura**: passa da circa **4-5** con un solo
segnale a **8-10** con quattro, e **non supera 12** nemmeno con quattro segnali
distinti su dodici celle — cioè anche mescolando famiglie diverse più di due terzi
delle celle sono ridondanti. L'incremento per segnale aggiunto è **decrescente**:
il salto da uno a due segnali è più grande di quello da tre a quattro.
*Falsificata se*: `N_eff` con quattro segnali distinti **supera 12**, cioè non
c'è nessun tetto e le dodici celle sono quasi indipendenti quando i segnali lo
sono — **oppure** se `N_eff` **non cresce affatto** passando da uno a quattro
segnali (resta entro ±1), nel qual caso il tetto non è una proprietà dei segnali
ma un artefatto dell'estimatore, e tutta la correzione per correlazione dei giri
82 e 85 va rifatta con uno strumento diverso.

**O11 — La curva del tasso di cambio riproduce i tredici candidati veri?**
Nata al giro 86. La curva del giro 83 è stata validata su **un solo** punto, e
quella validazione conteneva un errore di conversione fra alfa aritmetico e
geometrico che ha prodotto una «cautela» quattro volte più grande del vero (giro
86). Un controllo su tredici punti invece che su uno lo avrebbe intercettato
subito, e non è mai stato fatto.
Da misurare, sul solo 1969-2009: per ciascuno dei **tredici candidati** (i dodici
del giro 72 più il 12-3 top-5), le sue coordinate vere — **alfa lordo geometrico**
`(1+CAGR_cand)/(1+CAGR_bench) − 1` e **rotazione vera** — e poi il margine che la
curva sintetica del giro 83 **predice** a quelle coordinate, contro il margine
**osservato**. Riportare l'errore di predizione candidato per candidato, il suo
segno, la mediana degli errori assoluti, e la correlazione fra errore e rotazione
e fra errore e tracking error.
*Predizione*: la curva è **distorta verso l'ottimismo** ma di poco: l'errore
mediano è **positivo** (il sintetico predice più di quanto il reale consegna) e
sta **fra +0,1 e +0,4 punti**, e la **mediana degli errori assoluti resta sotto
0,5**. L'errore **cresce col tracking error** — correlazione sopra **+0,4** — che è
il meccanismo isolato al giro 86, e **non** cresce con la rotazione una volta
tenuto conto dell'alfa, perché la rotazione è già dentro la curva.
*Falsificata se*: la mediana degli errori assoluti supera **1,0 punto**, cioè la
curva non descrive le strategie vere e il numero operativo del giro 83 va ritirato
— **oppure** se l'errore mediano è **negativo**, cioè il sintetico è
*pessimista* e le strategie reali fanno meglio del modello, nel qual caso c'è un
canale di vantaggio che il sintetico non rappresenta e va cercato.
